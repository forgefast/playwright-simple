...
# -*- coding: utf-8 -*-
"""
Teste completo com gravação de vídeo usando ULTRA FAST MODE (Nova Arquitetura): Geração e Reprodução de YAML

Este script executa o ciclo completo com gravação de vídeo usando ULTRA FAST MODE,
usando a biblioteca diretamente (sem CLI) com a NOVA ARQUITETURA (RecorderFacade):
...
ARQUITETURA: Usa RecorderFacade (nova arquitetura baseada em serviços)
"""
...
YAML_PATH = project_root / "test_new_arch.yaml"
...
def get_logger() -> Optional[UnifiedLogger]:
    ...
    if _logger is None:
        _logger = UnifiedLogger(
            ...
            console_level="INFO",
            file_level="DEBUG",
            log_file=project_root / "logs" / "test_new_arch.log",
            json_format=False,
            console_output=True
        )
    return _logger
...
async def run_generation():
    """Executa a geração do YAML via recorder direto usando ULTRA FAST MODE (NOVA ARQUITETURA)."""
    import time
    start_time = time.time()
    
    print_step(1, "GERANDO YAML VIA RECORDER FACADE (ULTRA FAST MODE - NOVA ARQUITETURA)")
    ...
    print(f"✨ NOVA ARQUITETURA: Usando RecorderFacade (service-based)")
    ...
    try:
        # NOVA ARQUITETURA: Usar RecorderFacade
        from playwright_simple.core.recorder.recorder_facade import RecorderFacade
        ...
        recorder = RecorderFacade(config=recorder_config)
...
async def run_reproduction():
    """Executa a reprodução do YAML com gravação de vídeo usando ULTRA FAST MODE (NOVA ARQUITETURA)."""
    import time
    start_time = time.time()
    
    print_step(2, "REPRODUZINDO YAML COM VÍDEO (ULTRA FAST MODE - NOVA ARQUITETURA)")
    ...
    # Executar reprodução usando RecorderFacade (NOVA ARQUITETURA)
...
async def main():
    """Executa o ciclo completo com gravação de vídeo usando ULTRA FAST MODE (NOVA ARQUITETURA)."""
    print_section("CICLO COMPLETO COM GRAVAÇÃO DE VÍDEO (ULTRA FAST MODE - NOVA ARQUITETURA)")
    print("📝 Este script usa a biblioteca diretamente (sem CLI)")
    print("✨ NOVA ARQUITETURA: Usando RecorderFacade (service-based)")
    ...
    # Resumo final
    print_section("RESUMO DO CICLO COMPLETO COM VÍDEO (ULTRA FAST MODE - NOVA ARQUITETURA)")
    ...
    if gen_success and repro_success and video_exists:
        print(f"\n🎉 CICLO COMPLETO COM VÍDEO (ULTRA FAST MODE - NOVA ARQUITETURA) EXECUTADO COM SUCESSO!")
