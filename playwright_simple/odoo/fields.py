...
# -*- coding: utf-8 -*-
"""
Odoo field interaction module.

Handles interaction with all types of Odoo fields.
"""
...
class FieldHelper:
    ...
    async def fill_field(
        ...
        if field_type == 'many2one':
            # For many2one, value should be search text
            return await self._fill_many2one_by_locator(field_locator, str(value))
        elif field_type == 'char' or field_type == 'text':
            return await self._fill_char_by_locator(field_locator, str(value))
        elif field_type == 'integer':
            return await self._fill_integer_by_locator(field_locator, int(value) if isinstance(value, str) else value)
        elif field_type == 'float':
            return await self._fill_float_by_locator(field_locator, float(value) if isinstance(value, str) else value)
        elif field_type == 'date':
            return await self._fill_date_by_locator(field_locator, value)
        elif field_type == 'datetime':
            return await self._fill_datetime_by_locator(field_locator, value)
        elif field_type == 'boolean':
            return await self._toggle_boolean_by_locator(field_locator)
        elif field_type == 'selection':
            return await self._select_dropdown_by_locator(field_locator, str(value))
        elif field_type == 'html':
            return await self._fill_html_by_locator(field_locator, str(value))
        else:
            # Default to char
            return await self._fill_char_by_locator(field_locator, str(value))
    ...
    async def fill_many2one(
        ...
        if is_label:
            ...
            return await self._fill_many2one_by_locator(field_locator, search_text, position)
        else:
            ...
            for selector in selectors:
                try:
                    ...
                    if await field.count() > 0:
                        return await self._fill_many2one_by_locator(field, search_text, position)
                except Exception:
                    continue
            
            return False
    
    async def _fill_many2one_by_locator(
        self,
    ...
    async def _fill_char_by_locator(self, field_locator: Locator, value: str) -> bool:
    ...
    async def _fill_integer_by_locator(self, field_locator: Locator, value: Union[int, str]) -> bool:
        """Fill Integer field using locator."""
        return await self._fill_char_by_locator(field_locator, str(value))
    
    async def _fill_float_by_locator(self, field_locator: Locator, value: Union[float, str]) -> bool:
        """Fill Float field using locator."""
        return await self._fill_char_by_locator(field_locator, str(value))
    
    async def _fill_date_by_locator(self, field_locator: Locator, date_value: Union[date, str]) -> bool:
        """Fill Date field using locator."""
        if isinstance(date_value, date):
            date_str = date_value.strftime("%Y-%m-%d")
        else:
            date_str = str(date_value)
        return await self._fill_char_by_locator(field_locator, date_str)
    
    async def _fill_datetime_by_locator(self, field_locator: Locator, datetime_value: Union[datetime, str]) -> bool:
        """Fill Datetime field using locator."""
        if isinstance(datetime_value, datetime):
            dt_str = datetime_value.strftime("%Y-%m-%d %H:%M:%S")
        else:
            dt_str = str(datetime_value)
        return await self._fill_char_by_locator(field_locator, dt_str)
    
    async def _toggle_boolean_by_locator(self, field_locator: Locator) -> bool:
    ...
    async def _select_dropdown_by_locator(self, field_locator: Locator, option: str) -> bool:
    ...
    async def _fill_html_by_locator(self, field_locator: Locator, html: str) -> bool:
        ...
        for record in records:
            ...
            for field_name_inner, field_value in record.items():
                if isinstance(field_value, dict) and 'type' in field_value:
                    ...
                    if field_type == 'many2one':
                        await self.fill_many2one(field_name_inner, value)
                    elif field_type == 'char':
                        await self.fill_char(field_name_inner, value)
                    elif field_type == 'integer':
                        await self.fill_integer(field_name_inner, value)
                    elif field_type == 'float':
                        await self.fill_float(field_name_inner, value)
                    elif field_type == 'boolean':
                        await self.toggle_boolean(field_name_inner)
                else:
                    # Default to char
                    await self.fill_char(field_name_inner, str(field_value))
