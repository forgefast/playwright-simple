...
# -*- coding: utf-8 -*-
"""
Teste completo com gravação de vídeo usando ULTRA FAST MODE (Arquitetura Antiga): Geração e Reprodução de YAML

Este script executa o ciclo completo com gravação de vídeo usando ULTRA FAST MODE,
usando a biblioteca diretamente (sem CLI) com a ARQUITETURA ANTIGA (Recorder direto):
...
ARQUITETURA: Usa Recorder diretamente (antes da migração para RecorderFacade)
"""
...
# Import UnifiedLogger for debug logging
try:
    from playwright_simple.core.logging.unified_logger import UnifiedLogger, LogContext
...
YAML_PATH = project_root / "test_old_arch.yaml"
...
def get_logger() -> Optional[UnifiedLogger]:
    ...
    if _logger is None:
        _logger = UnifiedLogger(
            ...
            console_level="INFO",
            file_level="DEBUG",
            log_file=project_root / "logs" / "test_old_arch.log",
            json_format=False,
            console_output=True
        )
    return _logger

def get_steps_count(recorder) -> int:
    """Helper function to get steps count from recorder safely."""
    try:
        if hasattr(recorder, 'yaml_writer') and recorder.yaml_writer:
            if hasattr(recorder.yaml_writer, 'steps'):
                return len(recorder.yaml_writer.steps)
            elif hasattr(recorder.yaml_writer, 'get_steps_count'):
                return recorder.yaml_writer.get_steps_count()
        return 0
    except Exception as e:
        logger = get_logger()
        if logger:
            logger.warning(f"Error getting steps count: {e}")
        return 0
...
async def run_generation():
    """Executa a geração do YAML via recorder direto usando ULTRA FAST MODE (ARQUITETURA ANTIGA)."""
    import time
    start_time = time.time()
    
    logger = get_logger()
    if logger:
        logger.info("Starting YAML generation", action_type="generation")
    
    print_step(1, "GERANDO YAML VIA RECORDER DIRETO (ULTRA FAST MODE - ARQUITETURA ANTIGA)")
    ...
    print(f"🏛️  ARQUITETURA ANTIGA: Usando Recorder diretamente")
    ...
    try:
        # ARQUITETURA ANTIGA: Usar Recorder diretamente
        ...
        initial_steps_count = get_steps_count(recorder)
        max_wait_attempts = 10
        for wait_attempt in range(max_wait_attempts):
            await asyncio.sleep(0.05)  # 50ms entre tentativas
            current_steps_count = get_steps_count(recorder)
            if current_steps_count > initial_steps_count:
                # Step foi adicionado, podemos adicionar áudio
                if logger:
                    logger.debug(f"Step added after {wait_attempt + 1} attempts", step_number=current_steps_count)
                break
        else:
            # Se não encontrou, aguardar um pouco mais
            await asyncio.sleep(0.1)
            if logger:
                logger.warning("Step may not have been added yet, continuing anyway")
        ...
        for wait_attempt in range(max_wait_attempts):
            ...
            if current_steps_count > initial_steps_count:
                # Step foi adicionado, podemos adicionar áudio
                if logger:
                    logger.debug(f"Type step added after {wait_attempt + 1} attempts", step_number=current_steps_count)
                break
        else:
            # Se não encontrou, aguardar um pouco mais
            await asyncio.sleep(0.1)
            if logger:
                logger.warning("Type step may not have been added yet, continuing anyway")
        ...
        for wait_attempt in range(max_wait_attempts):
            ...
            if current_steps_count > initial_steps_count:
                # Step foi adicionado, podemos adicionar áudio
                if logger:
                    logger.debug(f"Password step added after {wait_attempt + 1} attempts", step_number=current_steps_count)
                break
        else:
            # Se não encontrou, aguardar um pouco mais
            await asyncio.sleep(0.1)
            if logger:
                logger.warning("Password step may not have been added yet, continuing anyway")
        ...
        for wait_attempt in range(max_wait_attempts):
            ...
            if current_steps_count > initial_steps_count:
                # Step foi adicionado, podemos adicionar áudio
                if logger:
                    logger.debug(f"Submit step added after {wait_attempt + 1} attempts", step_number=current_steps_count)
                break
        else:
            # Se não encontrou, aguardar um pouco mais
            await asyncio.sleep(0.1)
            if logger:
                logger.warning("Submit step may not have been added yet, continuing anyway")
        ...
        if generated_yaml.exists():
            ...
            # Log YAML generation success
            if logger:
                final_steps_count = get_steps_count(recorder)
                logger.info(f"YAML generated successfully", 
                          step_number=final_steps_count,
                          yaml_path=str(generated_yaml),
                          file_size=generated_yaml.stat().st_size)
            ...
            try:
                ...
                yaml_valid = False
                if logger:
                    logger.error(f"Error validating video config: {e}")
            ...
            if logger:
                logger.info(f"YAML generation completed", duration_ms=(elapsed * 1000), success=yaml_valid)
            
            return True, yaml_valid
        else:
            print(f"\n❌ YAML não foi gerado!")
            if logger:
                logger.error("YAML file was not generated")
            return False, False
            
    except asyncio.TimeoutError as e:
        print(f"\n⏱️  Timeout ao gerar YAML")
        if logger:
            logger.error(f"Timeout during YAML generation: {e}")
        return False, False
    except Exception as e:
        ...
        traceback.print_exc()
        if logger:
            logger.error(f"Error during YAML generation: {e}", error=str(e))
        return False, False
...
async def run_reproduction():
    """Executa a reprodução do YAML com gravação de vídeo usando ULTRA FAST MODE (ARQUITETURA ANTIGA)."""
    import time
    start_time = time.time()
    
    logger = get_logger()
    if logger:
        logger.info("Starting YAML reproduction", action_type="reproduction")
    
    print_step(2, "REPRODUZINDO YAML COM VÍDEO (ULTRA FAST MODE - ARQUITETURA ANTIGA)")
    ...
    # Executar reprodução usando Recorder diretamente (ARQUITETURA ANTIGA)
    ...
    try:
        ...
        if logger:
            logger.info("YAML reproduction completed successfully")
        ...
        if video_exists:
            ...
            if logger:
                logger.warning(f"Video not found in: {VIDEOS_DIR}")
            return True, False
        ...
        traceback.print_exc()
        if logger:
            logger.error(f"Error during YAML reproduction: {e}", error=str(e))
        return False, False


async def main():
    """Executa o ciclo completo com gravação de vídeo usando ULTRA FAST MODE (ARQUITETURA ANTIGA)."""
    logger = get_logger()
    if logger:
        logger.info("Starting complete cycle with video recording", action_type="main")
    
    print_section("CICLO COMPLETO COM GRAVAÇÃO DE VÍDEO (ULTRA FAST MODE - ARQUITETURA ANTIGA)")
    print("📝 Este script usa a biblioteca diretamente (sem CLI)")
    print("🏛️  ARQUITETURA ANTIGA: Usando Recorder diretamente")
    ...
    # Resumo final
    print_section("RESUMO DO CICLO COMPLETO COM VÍDEO (ULTRA FAST MODE - ARQUITETURA ANTIGA)")
    ...
    if gen_success and repro_success and video_exists:
        print(f"\n🎉 CICLO COMPLETO COM VÍDEO (ULTRA FAST MODE - ARQUITETURA ANTIGA) EXECUTADO COM SUCESSO!")
        ...
        if logger:
            logger.warning("Complete cycle finished with problems",
                         gen_success=gen_success,
                         repro_success=repro_success,
